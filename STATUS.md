...
- **เฟส Ops กำลังทำ (Epic MAR-80, spec ใน SPEC.md "เฟส Ops", 8 tasks Ops-T1..T8 = MAR-81..88):**
  ...
  - ⬜ Ops-T8 (HPA on catalog per Fable + metrics-server + k8s smoke + CMN1 activation staged) — ยังไม่เริ่ม (cluster พร้อม, port-forward Prometheus/Grafana/Kong ยังเปิด)
