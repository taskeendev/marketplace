...
- 🧹 **ponytail-audit ทั้ง codebase (2026-07-07)** — audit over-engineering 11 repos (workflow 69 agents, ทุก finding ผ่าน adversarial verify): verdict **APPROVE, codebase lean** (~15k LOC เจอตัดได้ ~422 บรรทัด + CI dedup potential ~430 + 1 dep). เปิดการ์ด `ai-review` 4 ใบ (To Do, ไม่ block อะไร): **MAR-112** CI/CD ซ้ำ ×7 repo → reusable workflow + Dockerfile เลิก hardcode jar · **MAR-113** web template leftovers (App.css 184 บรรทัด, dead assets, tw-animate-css, −253/−1dep — เสี่ยงต่ำสุด เริ่มก่อนได้) · **MAR-114** Java dead code/fields 7 services (เด่น: payment GET /{id} ไม่มี caller, ⚠️ F-29 BANNED enum เช็ค DB ก่อนลบ) · **MAR-115** gateway/deploy (ngx.base64 แทน b64 เขียนมือ, extract render-kong.sh, k8s เก็บกวาด). ที่จงใจ**ไม่**เก็บ: pom/application.yml/SecurityConfig/Testcontainers duplication (microservice convention) + CartService qty guards (validation ห้ามลบ)
